...
- Outstanding user smoke test: face relics + Madness checklist (DEVLOG archive Sec 5.7) - still not run.
...
Everything above is from builds and file reads that actually ran; the build is 0 errors and deployed. **No in-game smoke test has been run this session.** StS2 launches via Steam DRM, its UI cannot be driven automatically, and a running game locks `Spire1.dll`, so the build must happen before launching. Per `../AGENTS.md` Sec 2 the visual/interactive smoke test belongs to the user. Worth eyeballing, in rough order of risk:
...
Consequence: `Colosseum`, `MaskedBandits`, `MysteriousSphere`, `Mushrooms [Stomp]`, `DeadAdventurer`'s elite, `MindBloom [I am War]` and `SpireHeart` are all **implementable now**. StS1 act encounter tables are already extracted in `DEVELOP.md` Sec 7d. This is the highest-value next milestone and it needs no new dependency.
...
1. **M2 monsters** (5.1) - the big one, now unblocked. Take the shipped-scene route first; it needs no art. Encounter tables are in `DEVELOP.md` Sec 7d. Start from `docs/BaseLib-API.md` Sec 2.11 (`CustomMonsterModel` - **SHIPPED**) and Sec 2.12 (`CustomEncounterModel`).
2. Resolve `N'loth's Gift` (5.4) and `Girya` (5.5) from `research/BaseLib-unused-surface.md` Sec 3 and Sec 4.
...
- Consequence: the whole 3.4.5 surface is now legal at runtime; `docs/BaseLib-API.md` Sec 9's skew table is history, not a constraint.

### 6.2 Single dependency, decided with evidence
- `mods/BaseLib/BaseLib.json` declares `dependencies: []` - the earlier "runtime deps BaseLib+RitsuLib" note in DEVELOP.md Sec 0 was simply wrong.
...
- First wave (opus5): GremlinLouseWriter died after 18m20s having written NOTHING - transcript shows solid extraction work, then death mid-synthesis. Root cause matches DEVLOG Sec 5.6: batching writes to the end. Two more socket deaths followed.
...
- BossesW delivered Guardian/Hexaghost/SlimeBoss (split = 1xSpikeSlimeL + 1xAcidSlimeL per bytecode, NOT x2) and flagged a real runtime crash in SlimeSplit: `new T {}` is canonical/immutable -> `CreatureCmd.Add` AssertMutable throws. Fixed: `(T)ModelDb.Monster<T>().ToMutable()` then set SpawnHp.

### 7.3 Main-agent fixes after writer termination (perf)
- JawWorm AI rewritten from bytecode getMove: band picker 25/30/45 + conditional sub-rolls (0.5625 after Bellow->56.25% THRASH; 0.357 after THRASH^2->35.7% CHOMP; 0.416 after CHOMP->41.6% CHOMP), first move CHOMP, per-turn cached sub-roll via `base.Rng.NextFloat()`.
...
- Fix: `Spire1Encounter` overrides `CustomRunHistoryIconPath/OutlinePath` to
  `res://Spire1/images/run_history/{id.ToLowerInvariant()}.png`; shipped 40 placeholder 1x1 PNGs
  (20 encounters x main/outline) in pck. GOTCHA: BaseLib hands us `Id.Entry` UPPERCASE
...
- Run-history icons are 1x1 transparent placeholders: invisible in library/history screens.
...
- Cards: 320 auto-mapped by normalized name + 11 manual (relic-styled cards, potion cards,
  beta-era cards) = **331/331** card_portraits at 250x190 + big 1000x760 (StS1 ships 500x380;
  scaled +/-2x bicubic).
- Relics: **36/38** classes mapped to relics/*.png (94x94), outline/ (94x94), largeRelics/
  (256x256, falls back to upscaling the small art when a large variant doesn't exist).
...
- Powers: 30 auto + 20 semantic substitutes (StS1 buff icons live only inside texture atlases,
  not standalone files) = 50 icons at 64x64 + 256x256, center-square-cropped from power-card
  illustrations.
...
- Security (`review-security.md`): **no Critical/High**. Low x3 - floating `Version="*"` deps
...
- Powers: TimeWarp counts statuses/curses too AND gives every monster +2 Strength on
  ...
  at 1 stack die without acting) - Transient now attacks 4x not 5x; Constricted damage is
...
- Act4 review fixes: CorruptHeart Debilitate no longer increments moveCount (vanilla early
  ...
  BURN_STRIKE intent shows x2 hits.
...
- **对我方差异化结论的修正**：1)"The Ending 第 4 幕"不再是独占卖点；2)"多人正确性"弱化为部分卖点（AFTP 联机可用但需全员同配置，平衡已有社区补丁；事件不同步卡死仍无人修）。剩余真实独占：**角色/玩家卡池/遗物层、选单地牢 selector UX**（生态评论区 Azusa 明确请求、无人做）、超越平衡补丁的联机正确性。另发现我方 The Ending 未实现三钥匙门禁（StS1 原版与 Act 4 Heart 都有）--若保留自研第四幕需补此保真缺口。
...
- **DEVELOP.md 改写**：vision/Sec 0/Sec 1（P1 互操作验证、P2 缺口收口、P3 层内 UX 三新里程碑）/Sec 2b（SUPERSEDED）/Sec 9（N'loth's Gift、FaceTrader、Madness 解锁路线）/Sec 10（AFTP 参考二进制位置）。

### 后续执行序（P1 -> P2 -> P3，见 DEVELOP.md Sec 1）
...
- WARN  **碰撞面 #1**：我方 fallback 的 The Ending（CorruptHeart/SpireShield/SpireSpear/HomeActs=[4]）与 Act4Heart 同内容双份并存--若用户同时启用我方幕与 AFTP 栈，会出现两个第四幕/两套心脏。P1 审计首项：确认 BossDiscoveryOrder/act 池在双装时的行为，必要时默认关停我方幕（config 门禁已具备）。
...
### 冒烟正面信号（崩溃前 42 层）
AFTP/Act4Heart/ActToggler2/MP-Rebalance + Spire1 五件套正常同载；随机到 Necrobinder 打出了 SPIRE1-MADNESS（共享卡池互通 [x]）；我方遭遇（FOUR_SHAPES/DARKLINGS 等）连续正常出怪战斗；休息点/商店/事件 handler 全部工作。
...
- **我方修复** `AutoSlayModdedScreenHandlersPatch`（--autoslay 门禁）：反射注册 NWheelSpin/NMatchAndKeep 屏 handler，等待滑入动画后调小游戏公开 Complete()（结果构造时已定，旋转纯演出）。PortalMapBuilder 无公开完成方法，暂不注册。**用户指令：此类兼容补丁随 Spire1 发布承载（AFTP 冻结），已写入 DEVELOP Sec 0。**

### 战果 #3：心脏已斩，结局链最后一环 = 我方不死补丁误伤脚本处决
heart4（MoveNext 转译生效后）：**第四幕全程自动驱动，CORRUPT_HEART_BOSS 4 回合斩杀** [x]。
...
- AFTP 音量数学（LinearToDb(vol^2)+offset）在正常输入只产生 -inf 不产生 NaN -> 排除。
- 首现位置紧贴 SpeedX AutoProceed 活动 -> 头号嫌疑 SpeedX 加速x音频 tween；次嫌 AutoSlay 注入的 999 Plating/200 Strength 触发的 UI 路径。待对照实验（禁 SpeedX 重跑一次）定罪。
- 另实测 AFTP pck 缺 `sfx/chosen/chosen_death.ogg`（加载失败 2 次）--部分 StS1 音效缺失即"听感不同"的构成之一；遗产幕内 AFTP 有意替换音频为设计行为。

### 对外沟通规则（用户指令）
- 所有对外 issue/评论/草稿**必须披露 agent 参与**（对齐 AFTP 作者自曝 AI 参与的标准）：注明研究/撰写由 ox-alpha agent 在 Twelve-eight 指挥下完成、证据来源。已回填 AFTP #10 正文与评论、MegaCrit 草稿，规则入 DEVELOP Sec 5。
...
- **seed P1SMOKE2 = Regent 全程胜利 exit 0**（多 seed 扫描第二角色覆盖 [x]；两角色两胜利链）。NaN 基线（SpeedX 全开）= 3250；ERROR 仅 2 条。
...
- **E1 判定：AutoProceed 洗清**。NaN 3250（全开）vs 3279（关 AutoProceed）无显著差异 -> 头号嫌疑转为 `turboEnabled` 的 10x 时间缩放 x 音频 tween（E2 可选：关 turbo 跑 10 分钟取前段计数即可定罪，无需跑完全程）。
...
- 用户目击 `SPIRE1-BANDAGE_UP`（包扎）无卡图。审计：card_portraits 333 张中 **302 张为 ~314B 纯色占位**（250x190 尺寸正确但无内容）--session 11.2 的"331/331 mapped"仅指文件存在，非真美术。big/ 目录同样为 3KB 占位（不可用）。
- 恢复源 = `.tmp/sts1full/images/1024Portraits/`（368 张 jar 解包图）：basename 直配 + 去符号 squeeze（j_a_x->jax 等）+ 全树兜底；10 张显式别名：per-color 共享 strike.png/defend.png、charge_battery->conserve_battery、wreathe_of_flame->wreath_of_flame、judgment->judgement、lessons_learned->lesson_learned。PowerShell System.Drawing 批量 500x380->250x190（脚本 `.tmp/restore-portraits.ps1`）。零残留。
...
- 修复：`.tmp/restore-big-portraits.ps1` 从 sts1full 500x380 原生分辨率重生成 302 张 big（重跑安全，仅动 <4KB）；同批补 DrugDealer 遗物 mutagenic_strength 三件套（relics/ + outline + big，StS1 名 mutagen）。commit `465efe9`。
...
- **待办（恢复账号后）**：1) 目验卡面大图（包扎/暴走/哨卫/缴械/燔祭/递归等）；2) seed 扫描 P1SMOKE4 起覆盖 Ironclad/Silent/Defect/Watcher 四角色完整胜利；3) run_history 110 张 70B 遭遇图标为已知低影响缺口（StS1 无官方图标源，宁缺勿造）。
...
- P1SMOKE6 三连回归：r1/r2 暴露 SimpleLoc 同源竞态第二形态 + PatchAll 一损俱损缺陷 -> IsEnabled 补丁重定向声明基类 NClickableControl + MainFile 逐类 try/catch 加固（3ebbab0）-> r3 **SPIRE1-DEFECT 全程胜利**（EXC:0）。覆盖进度：Ironclad[x] Watcher[x] Defect[x]，仅剩 Silent。
...
- **尘封魔典（DUSTY_TOME）机制定罪**（用户报告"发的牌是封印王座"）：
  ...
  - 修复候选（待实证后决策）：1)给每角色配置一张忠实 Ancient 卡 2)SetupForPlayer 空集 fallback 补丁 3)文档化限制。
...
- **商店守卫终版生效**：`ShopEnoughGoldGuardPatch`（MerchantEntry.EnoughGold postfix）--SOZU 在手时药水槽 reason=sozu-ban、被跳过；含药水商店流程秒过。前两代实现全部废弃并记录：1)HandleAsync 前缀 .Wait() 主线程死锁（用户目击"对对碰"画面冻结）；2)CreateCard+AddGenerated 异步管线在 postfix 上下文 await 不恢复。
- **尘封魔典遗物链全通**：`DebugRelicInjectPatch` 挂 NMapScreen.Initialize（战斗外安全点），`relic.ToMutable()` + DustyTome 先 `SetupForPlayer(player)` -> `RelicCmd.Obtain` 成功无失败日志；sozu-ban 即其铁证（禁令 hook 需遗物真实入包）。教训：1)discarded task 吞异常必须 ContinueWith 记录；2)canonical 模型不可直接 Obtain/CloneCard（"used in incorrect place"）。
...
- **最终验证局**：SPIRE1-IRONCLAD *胜利 F17-A3，ERR:0 EXC:0 NaN:0，无注入残留 [x]。
...
- 我方排除依据：1)分歧堆属于第三方角色；2)触发卡为官方状态牌；3)我方 dll 双侧一致且无任何抽牌管线 hook；4)本局我方卡（RECYCLE/SUNDER 等）执行序列双侧一致。
...
- 处置建议：1)最小化 mod 集复测（BaseLib+Spire1），复现->上游 bug，消失->二分定位（先砍 MPLB）；2)统一双方 BaseLib 为工坊版；3) Sunder 的 Canceled 模式与官方 HandOfGreed 同构（击杀后 GainEnergy），暂不改码，若最小集下仍复现再做无等待重排。
...
- 原始 110k 行现场日志已被轮转丢失；现存日志仅剩自救痕迹（winx3/block 222 均为事后戳醒动作，非病因）。我方 RestSiteBackgroundPatch 只作用于 Spire1Act 已排除。
...
- **ProjectCritic**（critique-20260825.md）：17 条问题 P1x2/P2x7/P3x8。总体评价工程素养高，但抓出两个真 P1。
...
- 二卷语义：research/sts1-kb/mechanics/（MechKB 产出 **119 条规则**，javap 字节码控制流提取；用户示例"开局抽牌 vs 消耗 vs 抽到自动打出"在 draw-exhaust.md Sec 6 有唯一确定裁决：三者不交错--初始抽牌为原子块、triggerWhenDrawn 仅五类牌、消耗链整批抽完后按队列位执行）。附 8 条任务书假设勘误（含"Havoc 属 triggerWhenDrawn"系讹传）。
...
- 实锤：Highx4（非pure分支丢孪生注入->CHEESE崩溃回归；HandshakeResult struct缺ref->联机放行从未生效；RestSite救援缺Owner/UniqueNameInOwner->无效；三死开关零消费者）+ Medx8 + Lowx8。
...
- 四路 reviewer 重推 DEVLOG 全部结论（引擎事实 28/30[x]、数值 19/24、修复声明无虚报、覆盖/联机 10/13）+ 主会话 16 条独立抽样。总报告 research/audits/reverify-20260826.md，四原始报告同目录 reverify-*-20260826.md。
- 推翻并当日修补（全部经 ilspycmd 反编译实锤 + RVFIX1 冒烟 exit 0）：1)联机放行 struct 缺 ref（补 ref，4 源证实从未生效）2)3a0de3d 误删非 pure 分支 Ironclad/Defect 孪生注入（恢复+删 PureSts1Adds 死代码+删 DarkShackles 双注入）3)Feed 谓词取反（去反对齐引擎）4)Maw NOM 击数 off-by-one（opening 计 turnCount）5)火堆救援补 Owner+UniqueNameInOwner 6)Armaments +3 Block 违官方回滚 7)三处代码注释订正。
...
- **C DARVx尘封魔典**：Remote 多出一个 DustyTome 顶掉 VELVET_CHOKER（#558）
...
### 产出
- **P6.2 全 9 项**（commit 079281b）：ShiftingPower participants 门控、SkipNode loc 键、min_game_version 0.111、Girya 挪表+占位符修复、zhs 36 条 flavor **逐字对齐 KB**（advisory 拦截了 subagent 自拟文案违反 AGENTS Sec 6--机械 join 修正）、#if DEBUG 隔离、LegacySaveCompatPatch 最小剥离版（归档 vs 剥离待用户裁定）、Akabeko 调研跳过（无渗漏面）
...
**修复**（照 Effective 模式）：DarvOnlyInLegacyActsEffective + LegacyEnemiesGiveClassicSlimedEffective（SP-only 合取）；调用点全换：DarvPatches x2、粘液族 x5、TimeEater。MP 恒走原版分支双端一致。构建 0 错误。
...
- defense-powers.md R01-R10：五层防御干预点；无实体玩家版(>1,改局部变量)vs怪物版
  (>0,改 info.output)；Buffer 逐源/Invincible 回合预算；4)层钩子吃穿透格挡后的余量。
...
- mechanics/loot-rewards.md L01-L07：药水稀有度 65/25/10（potionRng）；宝箱
  treasureRng；商店价 = getPrice(rarity) x merchantRng.random(0.9,1.1)，基础价
...
- Splash（修复 #10，DEVLOG:740）：原实现 list.Remove(owner.CardPool) 按池**对象**排除；
  SharedCardReuse 令"可调用集合" contains "自己池对象"，移植 Defect 从"其他角色"选出自己已有
...
一条）；D6 开发模式四件套（已成卷）。
后续深挖候选：1)StS2 原生"跨池选牌"卡全量清单（哪些卡用了池并集/差集，逐一标注算
法定义）；2)ModHelper.ConcatModelsFromMods 的注册顺序与去重语义；3)StS2
...
- 续3补：评审门机械子集固化为 tools/semantics-audit.mjs（P4+P1+I7 一键运行；
  ...
  下一方向（按序）：1)DualWield 定向验证+修复（实现会话：控制台 card play -> 补两键 -> 冒烟）
  2)不变量目录继续扩充（StS2 联机池快照、事件卡牌来源审计）
  3)G2 清单与 tools 的联动已闭环，等待下一次实机事故回填。
...
- 续3四补：I12 联机同步拓扑+checksum 纪律（ChecksumTracker 全文：host 比对/20 条滚动窗/
  ...
  O08（五珠数值与 StS1 同源、Glass 新珠 evoke=Passivex2、Plasma 豁免 ModifyOrbValue、
...
- **队列**：1)I15 联机奖励分配专卷 2)StS1 剩余事件抽查（12 个）3)StS1 逐怪数据层 v1
  （getMove 内部语义，非本会话）4)不变量回填（待下次实机事故）。
...
本会话(zcode 审阅线)产出:
- 批判审阅定稿 `docs/CODE-REVIEW-20260904.md`(发现 R1-R15;P0x2 在 Pandora 修复
...
(P0x2 -> R1 -> R7/R5 -> R6/R8 -> P2/P3 -> PLAN)、待验证队列、工具清单、环境硬约束、
KB 指南、开发纪律、事故史速查、omp 第一小时清单、禁区。代码基线 `9453ebc`
(其后的提交均为文档/KB/工具),**已部署 dll 仍含两个 P0,omp 第一动作 = R2->R3->
构建部署冒烟**。

## Session 31 - 2026-09-06 晚 - omp 接手：R 波修复实施（P0x2 + P1 主批落地）
...
- **R10/R15/R6/R8 处置决策**记入 DEVELOP Sec 9（R10 deferred 待 javap 条件真值表；R15 接受
...
- **R9**（已验收合入，`2dab66d`）：`ThreeCultistsEncounter`（3xCultist，坐标 -465/-20、
  ...
  与引擎默认 10-20 一致免覆写；Sec 7d Act-2 现按 19/19 一一对应。子代理自跑 scoped 编译 0 错 0 警。
...
- **未做实机冒烟**：R1 三态（有钥匙/无钥匙/纯三幕）、R2 混沌局 Pandora 种子 sweep（交接
  文档 Sec 5 待验证队列）、R9 新遭遇生成、R12 Fission 出牌、R6 四新卡。游戏进程当时未运行，
  部署已完成；下一局 autoslay/真人局覆盖。**玩法语义偏差不在 autoslay 检出域（G4），
  R1/R12 需真人局 eyeball。**

## Session 31.5 - 2026-09-06 - language-hygiene scrub (repo-wide)

User directive: no non-approved-script characters may remain in on-disk
content (approved set = Chinese/English/French/German/Russian text plus
ASCII control and ASCII punctuation). Applied .tmp/scrub-illegal.mjs:

- 12007 flagged chars replaced in 639 files under the repo (excluded
  trees below). Map: arrows -> "->", em/en dashes -> "-", ellipsis ->
  "...", multiplication sign -> "x", curly quotes -> ASCII quotes
  (JSON-escaped), checkboxes -> [x]/[ ], section sign -> "Sec ", circled
...
- EXCLUDED (vendored upstream material, left verbatim):
  research/BaseLib-StS2 (BaseLib source incl. its jpn/kor localization),
...
- All JSON re-validated after scrub. Build/deploy refreshed, mechanical
  gates re-run green. New pure-English handoff:
  SESSION-HANDOFF-20260906-EN.md (this file superseded as recovery
...
## WS-0920-01 工坊三次发布 (1.2.0 -> 1.2.1 -> 1.2.2): 变更记录截断与格式修复

**结论**: 线上 Spire1 变更记录已修复. 当前工坊版本 **1.2.2**, 内容与 1.2.0 相同
(无代码/玩法变更), 三次发布只为让元数据真正重传.

### 起因: 线上变更记录被截断

抓取 `sharedfiles/filedetails/changelog/3799031900` 发现 v1.2.0 条目停在
`(2) 术语更正:Frail=脆弱(原误作\` -- 后续整段丢失.

**根因**: 文案里用了反斜杠转义的引号 `\"`. steamcmd 的 KeyValues 解析器**不解析该转义**:
`\` 是普通字符, 其后的 `"` 仍然**终止字符串**, 于是"值在此结束", 剩余正文变成**键名**.
两种后果:

| 情形 | 后果 |
|---|---|
| 截断后"键名"较短 | **静默截断**, steamcmd 照样报 OK (本次 Spire1) |
| 截断后"键名"过长 | **硬失败** `key name too long` -> 整个会话中止 (AAR 当时) |

### 为什么本地已修好却发不出去

**Steam 只在内容清单变化时才追加变更条目**. Spire1 的 payload 自 09-17 14:28 起字节未变,
所以 09-18 与 09-20 的 **5 次推送全部** `No content change detected`, 元数据不重传,
线上那条截断条目**一直保留**.

**教训**: `8/8 OK` 的退出码**不能**证明元数据已更新 -- 必须**抓取线上页面**核对.

### 三次发布

| 版本 | 目的 | 线上结果 |
|---|---|---|
| 1.2.1 | 修复截断: 新 changenote 说明原因并重新发布完整 v1.2.0 原文 | 条目**完整** |
| 1.2.2 | 修正 1.2.1 的格式错误 | 正确渲染 |

**1.2.2 的原因**: 1.2.1 文案误用 Markdown `**加粗**`, 而 Steam 只渲染 BBCode,
6 个星号被**原样显示** -- 而那段文本正在重新发布的 v1.2.0 原文里就写着
"修正 11 处星号错位(会渲染出字面星号)". 变更记录**追加式且不可编辑**, 只能发新条目覆盖.

### 顺带发现并修正的错误

**已提交的 payload 是过期的**: 我最初以为重建只是"版本号不同", 核实后发现

| | 观者衍生卡纹理 | INFUSED_CORE |
|---|---|---|
| 已提交 payload (09-15) | 存在 | 缺失 |
| 重建 payload | 已移除 | 存在 |

即已提交的 pck/dll 停留在 **09-17 那次源码提交之前**(`InfusedCore` + 移除观者衍生卡).
而**线上内容本就是重建后的树** -- 线上条目显示 29.446 MB, 与重建总大小**完全一致**.
所以本次不是改变玩家体验, 而是让仓库 payload 与玩家已有内容一致. 我已更正提交信息
(原写"byte-equivalent"是错的)并把 pck 一并纳入提交.

### 验证 (全部实测)

- `Uploaded new content ( ManifestID 7828035588746450380 ) for item 3799031900` (1.2.2)
- 抓取**原始 HTML** (`:raw`) 核对: v1.2.2 条目 `<b>` 标签 3 个, 字面 `**` **0 个**
- **注意**: reader-mode 输出会把 `<b>` 转成 `**`, 曾让我误判 1.2.2 没修好 --
  判定线上 BBCode 渲染**必须看原始 HTML**
