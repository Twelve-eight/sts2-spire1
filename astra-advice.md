...
## 附录: 审查早退义务与附加状态, 不只看改动行

通用流程见 [总建议附录](../astra-advice.md).

- **把失败改成成功时, 追被跳过的后半段**. 对每个 return 写清 "哪些安全条件已验证, 哪些尚未验证". 覆盖 ModMismatch 的结果不等于引擎继续执行后面的 hash 检查. 至少测一次两种失败同时成立.
- **清缓存之前先确认字段是不是事实来源**. `_keywords` 保存外部附加的状态, 不只是 CanonicalKeywords 的副本. 问 "除了这次升级还有谁写过它", 再决定整体重建还是只改属于自己的差量.
- **删功能要从注册反向追可达性**. 源码文件还能编译不等于它不会出现. 沿 autoAdd/池/幕列表/默认构造追到最终候选, 特别检查用户已取消的剧情/地牢是否仍从普通入口进入.
- **不要让调试改变研究对象**. 池盘点可能触发 lazy freeze; autoslay immortality 会屏蔽掉血/死亡. 先列出验证工具改变了哪些条件, 再判断测试能否说明真实行为.
- **兼容性由最终池与实际 DLL决定**. 查到旧同名文件只是定位线索, 不是当前行为证据. 已删除方向的旧文档只能帮助解释历史, 不能自动复活为本轮任务.

最短复验: 清单/hash 同错且 hash 放行关闭 -> 带外部 Retain 的卡升级 -> 普通事件最终候选 -> 非目标角色/官方池不变. 这是四个不同保证, 不用一个全局 AutoSlay 成功替它们验收.
