...
- 结果:三幕 boss(index 2)落 `2>=2` -> **零奖励,与钥匙无关**;Act4Heart 全栈
  ...
  处理,同一改写套在发奖口上方向相反。
- **修复**(遵守 Sec 0"生态补丁进本仓"指令):`RewardsSet.WithRewardsFromRoom` postfix--
...
- 根因:8781855"57 张官方孪生逐字段核对"实际只对带注释的前段执行了逐字段核对;
  ...
  DEVELOP Sec 7a 的陷阱清单(Defect writers 结论)是对的,两次修复波没有交叉引用它。
...
从未做过的审计方向。KB 全集(排除 deprecated)对比 我方类U复用通道,按颜色:
...
- GREEN/PURPLE 名义缺基础打击/防御(改名实现,实际全覆盖)[x]。
- 其中 9 张引擎已有同名卡(Hemokinesis、Offering、PerfectedStrike、Shiv、Slimed、
  Apotheosis、Discovery、HandOfGreed、TheBomb)-> 若字段一致应进 Reuse(Sec 7a),
  若不一致应自建--现状两头都不占。
...
- **R9 遭遇表缺口**:Sec 7d 权威表 Act2 strong 的"3 Cultists"无实现
  (Encounters/ 无 ThreeCultists*);"Shelled Parasite and Fungi"未组队
  (ShelledParasiteEncounter 单怪)。其余各幕 weak/strong/elite/boss 与 Sec 7d 一一对应 [x]。
- **R10 条件事件无出现条件**:Sec 7c 标记为"run 条件门控"的事件(AccursedBlacksmith、
...
- 基建:两件初始圣遗物与引擎同名(BurningBlood 逐行同机制;RingOfTheSnake 引擎走
  `ModifyHandDraw`、我方走 `BeforeCombatStart` 独立抽 2)--按 Sec 7a 复用或在 doc comment
  写明差异字段;Spire1LegacyPool 注释与 Strike/Defend 变体仍挂活跃池矛盾;
  DEVELOP Sec 2c 配置表与 Spire1Config 实际(PureSts1Pools/DebugShowLocKeys/
  IgnoreMpModDifferences/EnableSkipNodeButton)漂移。
- 卡牌:7 张自建同名卡缺 Sec 7a 差异字段 doc comment(BiasedCognition/Defragment/
...
- 数据:Sec 7c 权威清单自身漏了 SecretPortal(真实一代三幕事件,已实现且带 FLAG:
...
- Harmony 22 个补丁目标全部存在于当前引擎版本,签名/ref/返回语义正确
  (interop 代理对照反编译逐一核过)。
- 事件幕门控:Act1/Act2/Act3 名单与 Sec 7c 完全一致(Sssserpent 即 Liars Game,
  命名差异);六大神龛全为共享 [x]。
...
| "M4 content essentially complete" | DEVELOP Sec 0/Sec 1 | **失真**:R6 覆盖缺口 ~40 卡;R8 遗物层未量化 |
| "counts corrected 2026-08-30: 306 cards" | DEVELOP Sec 0 | 文件数属实;但"306"不等于"一代全集" |
...
| "event act gating 权威清单" | DEVELOP Sec 7c | Act1/2/3 名单成立;**清单自身漏 SecretPortal** |
