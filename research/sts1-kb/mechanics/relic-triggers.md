...
## 本卷范围
对 jar 内**全部 190 个遗物类**（`javap -p` 全量 + 签名级正则，工具 `../scan-hooks.mjs relics`）按钩子归类建档：战斗开场两段、回合两段、回合尾、出牌计数族、洗牌/弃牌/消耗、受击族、死亡/胜利、血量阈值、checkTrigger/onTrigger 特例、onEquip。成员清单即数据本体；计数遗物的通用模式已录 triggers.md Sec 4（计数点在 onUseCard 构造期），此处不重复。
...
```
**R02 atBattleStartPreDraw（开局"抽牌前"段）- 5 个** - 出处同 R01（调用点 turn-phase.md R01 步骤4)）。置信度：**高**
```
...
```
**R05 onPlayerEndTurn（回合尾哨兵链第一段）- 6 个** - 出处同 R01（turn-phase.md R07 步骤1)）。置信度：**高**
```
...
**R06 onUseCard（UseCardAction 构造期，计数遗物主入口）- 16 个** - 出处同 R01（模式详见 triggers.md Sec 4）。置信度：**高**
```
...
onBloodied:             MeatOnTheBone, RedSkull（血量阈值沿，damage-pipeline.md R04 步骤13)）
```
**R12 onMonsterDeath - 2 个**：`GremlinHorn, TheSpecimen`（怪物 die() 内同步直调，triggers.md Sec 5.1）。
