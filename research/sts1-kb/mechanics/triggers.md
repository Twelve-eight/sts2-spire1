...
| atEndOfTurn(boolean) | `(boolean)void` | 回合尾（见 Sec 6.4） |
...
| 方法 | 说明 |
|---|---|
| damage(DamageInfo) | 受击入口，内含受击钩子序（Sec 7） |
| takeTurn() abstract | 怪物回合（turn-phase.md 范围） |
| die() / die(boolean triggerRelics) | 死亡处理（Sec 5） |
| updateDeathAnimation() private | 死亡动画计时->isDead->胜利判定（Sec 5） |
...
| 字段 halfDead/isDying/isEscaping/isDead/escaped | 死亡状态机四元组（Sec 5.3） |
...
GameActionManager#getNextAction（cardQueue 分支）:
 1) player.powers      -> p.onPlayCard(card, monster)          [offset 622]
 2) 每个 monster m     -> m.powers 全部 p.onPlayCard(card,monster)[659]
 3) player.relics      -> r.onPlayCard(card, monster)          [697]
 4) player.stance      -> stance.onPlayCard(card)              [723]
 5) player.blights     -> b.onPlayCard(card, monster)          [786]
 6) hand.group 逐卡    -> c.onPlayCard(card, monster)          [855]
 7) discardPile 逐卡   -> c.onPlayCard(card, monster)          [924]
 8) drawPile 逐卡      -> c.onPlayCard(card, monster)          [993]
 9) cardsPlayedThisTurn/combat 登记
 10) player.useCard(card, monster, energyOnUse)                [1588]
```
- fizzle 特例：目标已死且 target==ENEMY -> 牌淡出（fadingOut+ExhaustCardEffect），跳过 6) 之后全部步骤（含 useCard）。主代理验证 + 本文复核 `[1482-1540]`。置信度：高。
...
5. **`new UseCardAction(card, target)` 构造 [4281-4285]** -- 构造函数本身触发一轮同步通知链（Sec 3）；随后 `actionManager.addToBottom(该实例)` [4286]。
...
AbstractRoom#update（isBattleOver && actions 空 && timer<=0 时） (AbstractRoom.jc:556-1018)
 ...
 -> 按房间类型 addGoldToRewards(...)：Boss=100+/-5(asc13+ x0.75)；Elite=treasureRng 25-35；
...
14. hp<=1 -> Sec 5.5 救援链（FairyPotion/LizardTail.onTrigger/DeathScreen）
```
...
- UseCardAction ctor：powers/relics/hand/discard/draw 五连 iterator 循环（Sec 2.3 行号）。
...
| 类别 | 机制 | 例证 |
|---|---|---|
| **同步直调** | getNextAction onPlayCard 全链（1)-8)） | GameActionManager#getNextAction |
...
- **R13** 受击钩子序（两侧一致的部分）：攻方改伤(relics->powers) -> 守方改伤(relics->powers) -> onAttack(攻方) -> onAttacked(守方) -> 掉血族(onLoseHpLast/onLoseHp/wasHPLost) -> onInflictDamage(攻方)；玩家侧守方在 onAttacked 之后按 powers->relics 排列，怪物侧无 relic。出处 Sec 7 两表。置信度高。
- **R14** 计数型遗物计数点在 onUseCard（或 Sundial 的 onShuffle）构造期同步递增，满额效果动作 addToBot 追加，因而执行序位于"卡效果之后、UseCardAction.update 之前"。出处 Sec 4 四例。置信度高。
- **R15** 同容器（powers/relics/orbs/blights/牌堆）多订阅者按获得顺序正序通知（ArrayList.iterator 正向遍历）；跨容器顺序由各调用点硬编码，互不相同。出处 Sec 8。置信度高。
...
5. Sundial 的 wiki 文本与字节码行为不一致（见 Sec 4.4），建议以字节码为准并回写勘误；其余计数遗物未逐一比对 wiki。置信度高（字节码侧）。
