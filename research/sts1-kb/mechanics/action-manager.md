...
| 级 | 容器 | 取件方式 | 说明 |
|---|---|---|---|
| 1) | `actions`（ArrayList） | `remove(0)` FIFO | 常规动作流 |
| 2) | `preTurnActions` | `remove(0)` FIFO | 回合起始缓冲（见 R05） |
| 3) | `cardQueue`（CardQueueItem） | 只看头部，每帧处理一件 | 打牌请求；`card==null` 为回合尾哨兵 |
| 4) | `monsterQueue` | 头部逐只 | 敌方行动（见 turn-phase.md R08） |
| 5) | -（turnHasEnded 标志） | 新回合序列 | 见 turn-phase.md R10 |
