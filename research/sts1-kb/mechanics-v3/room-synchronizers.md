...
- **生成一致**：BeginRelicPicking（L89-132）用**共享 `_rng`**（构造注入的 run 级 Rng）+ **RelicGrabBag 共享抓包**（RunState 级，CombatStateSynchronizer 每次 sync 时 host 下发快照强制对齐，卷四 Sec 5）-> 双端 roll 出**同一组**遗物。**宝箱遗物双端一致是引擎强保证，不是巧合**。
...
- 抓包状态序列化进 run 存档；MP 每次房间过渡 host 下发快照（CombatStateSynchronizer._sharedRelicGrabBag，卷四 Sec 5 表格 Run 层）。
