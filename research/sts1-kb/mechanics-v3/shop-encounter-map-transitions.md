...
- 遗物货架走 RelicGrabBag `PullFromBack`（卷五 Sec 4：商店专用从桶尾拉，不抢奖励位）。
...
### 3.4 RunLocation 与位置定向缓冲（卷四 Sec 3 补充）
...
| 死等点 | 同步器 | 解除条件 | 断线兜底 |
|---|---|---|---|
| WaitForSync（卷四 V4-R6） | CombatStateSynchronizer | 收齐玩家数据+RNG 快照 | [x] OnPeerDisconnected L103-111（+RNG 快照缺失则 client 仍挂，卷四已订正） |
| AfterAllRestSitesCompleted（卷五） | RestSiteSynchronizer | 全员 completionTaskSource | [x] OnPeerDisconnected L144-162 |
