...
3. **池替换是全局的,起手替换是 per-character 的**--两个语义不能搞混(见 Sec 2.3,Spire1 桥接曾因此返工)。
...
- **起手替换是 per-character**:`ReplaceStartingDeck(character)` 查 `IsCharacterRunActive(character) && ActiveReplaceStartingCards`,产 `ChaosCardRegistry.Canonical(character, slot) x BasicCountFor`。
...
- Prefix 我方三角色 `StartingDeck` getter:per-character + `ActiveReplaceStartingCards` -> `Canonical x BasicCountFor`(同构 ReplaceStartingDeck)。
...
- **数值**(dolso 配置):金属化 = act_multxact + add = 2xact+2(act1=4... 实测 17 与高进阶/多幕一致)。
...
| 档 | 模式 | 例子 | 联机安全 |
|---|---|---|---|
| A | 纯种子派生+双端同源(不读本地配置/本地状态) | 引擎地图拓扑、AutoAnthony se_buff 的 RNG 部分 | [x] 天然一致 |
| B | 经网络同步面同步的 mod 状态 | AutoAnthony 池快照(ModifierModel)、AFTP 事件选项消息 | [x] 契约内一致 |
