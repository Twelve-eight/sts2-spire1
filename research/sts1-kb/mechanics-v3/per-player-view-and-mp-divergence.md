...
> **规则 V4-R4**：mod 拿奖励不要绕过 `Reward/RewardsSet` 体系直发 `CardCmd.Add`--直发不经同步器，双端各自改牌库=checksum 分歧（除非该路径双端从同一消息驱动，见 Sec 4）。
