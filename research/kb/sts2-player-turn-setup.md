...
```
1) 守卫：creature 死 / PlayerCombatState null -> return
2) if (Hook.ShouldPlayerResetEnergy(state, player)) -> ResetEnergy()（+gain_energy 音效）
   else                            -> AddMaxEnergyToCurrent()   <- * 额外回合分支：叠加不重置
   -> Hook.AfterEnergyReset
3) Hook.BeforeHandDraw(state, player)
4) handDraw = Hook.ModifyHandDraw(state, player, 5m, out modifiers)   <- * 默认抽 5（常量直证）
...
6) await CardPileCmd.Draw(ctx, handDraw, player, fromHandDraw: true)
7) Hook.AfterPlayerTurnStart(state, ctx, player)
```
...
- Innate 抽满保证是**引擎级**（c 项 max 修正），StS1 同语义在 `CardGroup#initializeDeck`（Innate/瓶装置顶 + 超出部分转 preTurnActions 补抽，draw-exhaust.md Sec 1.1）--实现路径不同（StS1 靠牌堆排序+补抽，StS2 靠加大抽牌数）。
