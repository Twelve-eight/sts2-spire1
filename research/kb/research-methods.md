...
> 目的：把字节码/反编译取证过程中的**可复用方法**与**实际踩过的坑**固化下来，让后续会话（含主会话续作、子代理分工）不再重复付学费。原则"skill 放方法、KB 放事实"在缺 skill 基建的本仓库落地为：本篇放方法，`mechanics/` 与 `kb/sts2-*.md` 放事实。按 AGENTS.md Sec 1，本篇同时是"下次该怎么做"的入口。
...
# 3) 引用方全量枚举：见 Sec 4 常量池扫描法
# 4) 全量钩子面扫描：research/sts1-kb/scan-hooks.mjs（见 Sec 5）
```
...
**P-M6 PowerShell 写文件加 BOM**（AGENTS.md Sec 10 已录，此处归档引用）：写 .java/.mjs 一律用 Write 工具或 `-Encoding ASCII`。
...
**M7 子串污染**：`includes("atEndOfTurn")` 会误命中 `atEndOfTurnPreEndTurnCards`；同理 `onAfterUseCard  contains  onUseCard`、`atDamageFinalReceive  contains  atDamageReceive`、`onAttackedToChangeDamage  contains  onAttacked`。**必须按方法签名正则匹配**（`/void atEndOfTurn\(boolean\)/`）。2026-09-05 首版扫描即因此出错后返工。
...
**M10 FIFO 推导三件套**：1)块级调用点偏移（`GameActionManager#getNextAction` 的新回合块 1983-2228、开局块、哨兵链）决定**入队先后**；2)`addToBot`=队尾、`addToTop`=队首（多次 addToTop 连用 = 逆序执行）；3)**构造期 vs 执行期**区分--`DrawCardAction` 3 参构造器 new `PlayerTurnEffect` 的瞬间就重置能量（视效类构造器藏逻辑）、`UseCardAction` 构造器同步发 onUseCard 钩子。渎神 vs 无实体旗舰仲裁就是这三件套推出来的（death-arbitration.md R17-R19）。
