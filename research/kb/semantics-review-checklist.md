...
> 目的：把 `pool-architecture.md` G2 固化为**可直接派发的评审提示**。适用对象：任何涉及卡池/注册表/跨角色取牌/资产管线/本地化变量的改动（新卡、新角色、池操作、兼容桥、事件奖励）。使用方式：主会话在合入前逐条自检，或把"评审提示词"原样派给 reviewer 角色子代理（提示词在 Sec 3，允许整段复制）。每条都给出"问题->要看的证据->放行标准"。
...
| # | 提问 | 证据要求 | 放行标准 |
|---|---|---|---|
| P1 | 候选集是**按 Id 集合差**定义的吗？ | `Remove(...CardPool)` 类池对象排除**仅允许**出现在其后紧跟 Id 级差集的补丁中（正例锚点：`Patches/SplashOwnSetSubtractPatch.cs` 45 行 Remove 后 48-53 行 ownIds 差集）；裸排除 = 不通过 | 有集合差实现，或改动不涉及候选集 |
...
然后对改动 <文件列表或 diff 范围> 逐条回答上面 Sec 1 P1-P8 与 Sec 2 M1-M4 的"提问"，
