...
StartTurn(side):
  ...
  1) 逐怪 BeforeTurnStart：power.AmountOnTurnStart = power.Amount   * 快照
  2) Hook.BeforeSideTurnStart
  3) 玩家侧：Phase=Start、能量/抽牌 Setup（SetupPlayerTurn 异步、联机 pause context）
     敌方侧：敌回合横幅
  4) 逐怪 AfterTurnStart：玩家首回合跳过 -> ClearBlock():
  ...
  5) 逐怪 Hook.AfterBlockCleared
  6) Hook.AfterSideTurnStart；玩家侧再逐玩家 OrbQueue.AfterTurnStart
  7) AutoPrePlay 阶段（AutoPrePlay -> CheckForEmptyHand -> AfterAutoPrePlayPhaseEntered -> Phase=Play）
```
