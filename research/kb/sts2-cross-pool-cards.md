...
vanilla 安全依赖两个隐含条件：1)CharacterCardPools 只含**角色专属池**（共享卡在无色池，天然不在候选）；2)`owner.Character.CardPool` 对象与该角色可调用专属卡集合相等。我方 SharedCardReuse 打破2) => 复刻此模式即复现事故（一代 Defect 从"其他角色"选出自己已有的官方 Defect 卡）。另注意 **`Count > 1` 护栏**：只有一个角色池时**不做排除**--vanilla 自己就接受"此时候选含自己的卡"，移植时必须显式决定我方策略（保持原样 or 恒差集）。
