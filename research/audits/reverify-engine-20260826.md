...
- 日期：2026-08-26　验证域：DEVLOG.md Sessions 4-7「Engine facts」段 + Sec 6.3 + Sec 7.4 + Phase3-4 + Sec 9.3
...
- 裁定口径：[x] 验证成立 / ? 验证失败（附反证）/ WARN  无法验证或部分成立（说明缺口）。所有行号为本次实测行号。

---

## 一、Session 4 Engine facts（DEVLOG L33、L36-46）

### A1 [L33] `RelicRarity.Event` 是池排除机制 -- [x]
...
### A2 [L37] TryModifyRewards 签名与语义 -- [x]
**证据**：
...
### A3 [L38] AfterGoldGained 零收益不触发 -- [x]
...
### A7 [L42] FromChooseACardScreen -- [x]
...
### A8 [L43] CardPilePosition 与 Random 语义 -- [x]
...
### A11 [L46] GetRelic<T> / Relics / RelicCmd 表面 -- [x]
...
### B2 [L234] DonorId 视觉替换 -- [x]（末句为判断性陈述，静态无法完全证实）
...
### B3 [L235] GenerateAllEncounters postfix -- [x]
...
### C3 [L281] RollMove 用 RunRng.MonsterAi；MonsterModel.Rng 可作子滚 -- [x]
...
### C4 [L282] 动画事实 -- WARN （API 半部 [x]，rig 清单半部不可验证）
- [x] `SetupAnimationState(MegaSprite controller, string idleName, string? deadName, bool deadLoop, string? hitName, bool hitLoop, string? attackName, bool attackLoop, string? castName, bool castLoop)` 存在于 CustomMonsterModel.cs 静态方法，签名形状与 DEVLOG 一致。
- [x] 未知触发 no-op + Log.Warn：`CreatureAnimator.cs:92-95` `if (!_spineController.HasAnimation(_currentState.Id)) { ... Log.Warn($"could not find '{_currentState.Id}' animation on '{value}'"); }`（排队路径 :125-128 同款）。
- WARN  fat_gremlin/lagavulin_matriarch/torch_head_amalgam/slimed_berserker 各自的轨道清单是资产级断言（.scn/.tres spine 数据），不在反编译 C# 可达范围内；本次不裁真伪，如需闭环须解包 pck 核对场景资源。

### C5 [L283] SetMaxAndCurrentHp 存在性 -- [x]
...
### D1 [L78] CanonicalVars/Keywords/Tags 每实例单次读 -- [x]
**证据**：
...
### D2 [L79] 缓存 CanonicalVars/DynamicVars 会成 bug -- [x]
...
### D3 [L80] Clash/SignatureMove 性能修复的行为等价性 -- [x]
**证据**：
...
### E1 文本路径 LocManager.GetTable / LocTable.GetRawText -- [x]
...
### E2 EncounterModel.MapNodeAssetPaths -- [x]
...
### E3 BaseLib RoomIconPathPatch 短路 -- [x]
...
- **通过率：28/30 = 93.3%**；? 1 条（B1b 排他量词，零下游影响）；WARN  1 条（C4 rig 轨道清单，资产级超出反编译可达域）。
