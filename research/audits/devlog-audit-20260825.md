...
| A3 | 08-21 | 用户要求三份库接口文档常驻 `docs/`（Sec 5.8 "Per user request"） | docs/BaseLib-API.md 等 3 份 | 已实现 | `wc -c` 实测 109997 / 93215 / 136786 字节，与 DEVLOG 记录逐字节一致 |
...
| A5 | 08-21 | 启动事故处置：3.3.5 备份目录移出 mods/（Sec 8.3） | 目录操作 | 已实现 | live `Slay the Spire 2/mods/` 下无 BaseLib-3.3.5-backup（ls 实测），仅 BaseLib/Spire1 等 |
...
| A8 | 08-23 | 用户决策：放弃自研地牢呈现，转向生态互补层（Session 14） | DEVELOP Sec 0 改写、`84bc1f9` 收编遗留 | 已实现且已执行到 P1 全链路 | `DEVELOP.md:8` "DIRECTION PIVOT (session 14, user decision)"；P1 冒烟见 DEVLOG Sec 15 战果#4 |
...
| A10 | 08-23 | 兼容补丁随 Spire1 发布承载、AFTP 冻结（用户指令，写入 DEVELOP Sec 0） | DEVELOP 条款 + AutoSlayModdedScreenHandlersPatch | 已实现 | `DEVELOP.md:11`；`Patches/AutoSlayModdedScreenHandlersPatch.cs:199-200`（含 TotalFloor 49->120 transpiler） |
| A11 | 08-23 | 对外沟通必须披露 agent 参与（用户指令，入 DEVELOP Sec 5） | DEVELOP 条款 | 已实现 | `DEVELOP.md:79` "(user directive ... ox-alpha ... Twelve-eight)" |
...
| A17 | 08-24 | SPIRE1-WATCHER 归档（用户指令：AFTP 生态已有成品 Watcher） | `f2f3305` 配置门禁 -> **`0c2ac26` 移除开关改永久隐藏** | 已实现但与 DEVLOG 记录漂移 WARN  | `Character/Watcher.cs:28-33` 注释 "ARCHIVED (**no config switch by user decision**)"，Hide=true 硬编码；`git log -S EnableSts1Watcher` 显示 f2f3305 加、0c2ac26 删 |
...
| B12 | N'loth's Gift "NO hook exists...不可实现"（Session 4） | **不成立（已被推翻）** | CardRarityOdds.cs:83 `public RollWithoutChangingFutureOdds`；DEVLOG Sec 13 AFTP Transpiler 实证；DEVELOP.md:9 | DEVLOG 自身在 Sec 5.4/Sec 13 已记录反转；代码注释仍陈旧（见 D2） |
| B13 | FaceTrader.getRandomFace=未拥有五脸按序收集+Circlet 兜底+miscRng.randomLong shuffle 取[0]；Trade 分支免费 | **成立（本次新鲜反汇编）** | jar `events/shrines/FaceTrader.class` getRandomFace 全量字节码（hasRelicx5->size==0 加 Circlet->new Random(miscRng.randomLong())->shuffle->get(0)）；Trade 分支仅 logMetricObtainRelic+spawnRelicAndObtain，无 damage/gainGold | 与 DEVLOG Sec 5.7 描述逐点吻合 |
...
| B42 | 遭遇账目：Act1 20 + Act2 17 + Act3 17 + Ending 2 = 56 | **小漂移**：盘上 55（act2=17、act3=16、act4=2，其余 20 无显式 HomeActs） | Encounters/*.cs grep HomeActs 统计 | 与 Sec 12.2 "weak variants deleted" 相符；Sec 12.1 的 "17 encounters" 未随之更新 |
...
2. **N'loth's Gift 可实现性（B12/D2）**：S4 断言"不可实现"已被 S5.4/S13 推翻（`CardRarityOdds.RollWithoutChangingFutureOdds` public 缝真实存在，本审计独立确认）。但 `Events/Nloth.cs:26` 类注释仍写 "NlothsGift is not implementable"--**注释陈旧**，与 DEVELOP Sec 0/Sec 9 的现行结论相抵。
3. **遗物中文本地化（B45）**：DEVLOG Sec 9.2 给人以"34/37 已映射"的进展印象；实际落盘为零（zhs/relics.json={}，代码零中文）。数据准备 != 交付。
4. **遭遇总数（B42）**：Sec 12.1 各幕合计隐含 56，盘上 55（Act3 少 1，系同日 Sec 12.2 删除弱变体所致，前文未回填）。
...
| U2 | S5 验证节 | 脸面具清单 1-6（FaceTrader 免费、GremlinMask 弱自身、FaceOfCleric 胜后+1、NlothsMask 金付 relic 无、SsserpentHead ?节点 50 金、WindingHalls 2xMadness） | **同上，无逐项实机记录**；代码层全部核实无误（B13-B17） |
| U3 | Sec 5.3 | RitsuLib character skins unproven `[INFERENCE]` | 随 Sec 6.2 弃用 RitsuLib 而**失效（问题消失）** |
| U4 | Sec 5.3 | FreePlayBindingRegistry ordering `[UNVERIFIED]` | 同上失效；Necronomicon 最终走引擎原生 `SetToFreeThisTurn`（CardModel.cs:1267-1271 已核） |
| U5 | Sec 6 验证节 | "NOT yet rebuilt with Exordium+patch" | 已闭合：S7 起 Debug+Release 构建绿并多次部署（20:59 等） |
...
| U12 | Sec 15 | SpeedX 联系作者、MegaCrit/Act4Heart 草稿递交 | 用户动作项；三份草稿齐备于 `.tmp/issues/` |
| U13 | Sec 5.5/代码 | Girya rest-site Lift 选项未接线 | FLAG 仍在：`Relics/Girya.cs:27`（被动版已有，成长途径缺失） |
| U14 | Sec 10.3 | AcidSlimeS maxRepeats=1 与 StS1 base 自由 50/50 的偏差 | LOW 挂账保留（cosmetic deviation） |
| U15 | Sec 10.3/待办 | run_history 110 张占位图标 | 已知低影响缺口（StS1 无官方图标源，宁缺勿造）；repo images/run_history 实测 110 文件 |
