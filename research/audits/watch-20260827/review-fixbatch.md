...
## 1. 删类联动：Vampires 引用官方 BloodVial - **正确 [x]**
...
## 2. Token 卡归档 Spire1LegacyPool（11 张）- **正确 [x]**
...
- 漂移主张经 dllsrc 反证：官方 Bludgeon=Uncommon（StS1 Rare[x]漂移）、Acrobatics=Uncommon（StS1 Common[x]）、Predator=Common（StS1 Uncommon[x]）。三条全部属实。
...
## 4. DustyTome 回退池过滤自研同名卡 - **正确 [x]**

- `DustyTomeAncientFallbackPatch.cs:58-63`：`Assembly.GetType("Spire1.Spire1Code.Cards." + c.GetType().Name, throwOnError: false) == null`，程序集取自本 patch 类型（Spire1 mod 程序集）[x]，`throwOnError:false` [x]。
...
## 5. Disarm DynamicVar 化 - **正确 [x]**

- `Disarm.cs`：`new DynamicVar("StrengthLoss", 2m)` 正向存储；OnPlay `PowerCmd.Apply<StrengthPower>(..., -loss, ...)` 取负应用（`StrengthPower.AllowNegative=true`，dllsrc 证实负值合法）；`OnUpgrade => UpgradeValueBy(1m)` -> 2->3 -> 应用 -3 [x]。
...
## 6. AFTP fork：RebalancedModeEffective + ClassicSlimedOnPlayPatch - **正确 [x]（含 2 条 P3 观察）**
...
- 单机局：`NetSingleplayerGameService.Type == Singleplayer` -> `Effective == RebalancedMode` -> 本地配置完全生效 [x]。
- 联机局（Host/Client）：Effective 恒 false -> 任何本地配置都走原版分支 -> 两端无条件一致 [x]（这正是 divergence #55/#35 的修复目标）。
- ClassicSlimedOnPlayPatch 对称：`Type != Singleplayer` -> 原版；Singleplayer -> 保留经典变体 [x]。

## 8. 药水删除 - **正确 [x]（1 条 P3 洁净度备注）**

- `mod/Spire1Code/Potions/` 仅剩 ExplosivePotion.cs / FearPotion.cs / Spire1Potion.cs。全仓 grep 无任何代码引用被删的 6 个药水类 [x]。
...
- 保留正确性：ExplosivePotion/FearPotion 在 dllsrc 全量搜索**无官方对应类**（mod 独有语义），必须保留 [x]。
- 孤儿键风险：eng/zhs cards.json:194-207 仍存 6 药水的 `SPIRE1-*` 键。`LocTable.MergeWith` 纯字典合并、`LocException` 只在 **missing** key 的 Get 上抛出--多余键是惰性死数据，**无任何报错路径** [x]。relics.json 同理（~40 个被删遗物键）。属可留可清的洁净度项（P3）。
